...
class Status < ApplicationRecord
  ...
  # Both sides are lowered rather than using ILIKE: the index behind this is
  # pg_bigm, which accelerates LIKE only and is case-sensitive, so the query has
  # to match index_statuses_on_text_bigm's lower(...) expression. Lowering the
  # pattern in SQL rather than Ruby keeps one definition of case for both sides.
  scope :matching_text, ->(query) { where('lower(statuses.text) LIKE lower(:pattern) OR lower(statuses.spoiler_text) LIKE lower(:pattern)', pattern: "%#{sanitize_sql_like(query)}%") }
